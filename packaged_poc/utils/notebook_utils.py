...
def load_and_set_env_vars(env: str) -> Dict[str, Any]:
    ...
    base_data_vars_vars_path = os.path.join(
        relative_root, "conf", ".base_data_params.env"
    )
...
def load_and_set_env_vars_with_project(env: str, project: str) -> Dict[str, Any]:
    ...
    base_data_vars_vars_path = os.path.join(
        relative_root, "conf", project, ".base_data_params.env"
    )
    dotenv.load_dotenv(base_data_vars_vars_path)
...
def load_config(task_name) -> Dict[str, Any]:
    """
    ...
    config_path = os.path.join(
        os.pardir, os.pardir, "conf", "pipeline_configs", f"{task_name}.yml"
    )
...
def load_config_with_project(pipeline_name, project) -> Dict[str, Any]:
    """
    Utility function to use in Databricks notebooks to load the config yaml file for a given pipeline
    Return dict of specified config params
    Parameters
