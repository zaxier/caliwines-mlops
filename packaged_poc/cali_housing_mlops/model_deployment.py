...
class ModelDeploymentConfig:
    """
    Configuration data class used to execute ModelDeployment pipeline.
...
class ModelDeployment:
    ...
    def _batch_inference_by_stage(self, stage: str) -> pyspark.sql.DataFrame:
        ...
        model_inference = ModelInference(model_uri=model_uri, input_table=self.cfg.reference_data)
    ...
    def run(self):
        ...
        with mlflow.start_run(run_name=mlflow_tracking_cfg.run_name):
            ...
            production_eval_metric = self._get_evaluation_metric(
                y_true=prod_inference_pred_pdf[self.cfg.label_col],
