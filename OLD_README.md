...
```bash
conda create -n packaged_poc python=3.9
conda activate packaged_poc
```
...
```
This command will create your personal repository under `/Repos/<username>/packaged_poc`.
...
- To trigger the release pipeline, get the current version from the `packaged_poc/__init__.py` file and tag the current code version:
```
